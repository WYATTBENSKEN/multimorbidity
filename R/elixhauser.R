...
elixhauser <- function(dat = NULL,
  ...
  ## ICD-9 or Both ----
    ...
    dat_comb <- dat_comb %>%
      dplyr::mutate(elix_chf = dplyr::if_else(.data$chf_ot >= 2 | .data$chf_ip >=1, 1, 0),
                    elix_valve = dplyr::if_else(.data$valve_ot >= 2 | .data$valve_ip >=1, 1, 0),
                    elix_pulmcirc = dplyr::if_else(.data$pulmcirc_ot >= 2 | .data$pulmcirc_ip >=1, 1, 0),
                    elix_periph_vasc = dplyr::if_else(.data$periph_vasc_ot >= 2 | .data$periph_vasc_ip >=1, 1, 0),
                    elix_htn_uc = dplyr::if_else(.data$htn_uc_ot >= 2 | .data$htn_uc_ip >=1, 1, 0),
                    elix_htn_c = dplyr::if_else(.data$htn_c_ot >= 2 | .data$htn_c_ip >=1, 1, 0),
                    elix_paralysis = dplyr::if_else(.data$paralysis_ot >= 2 | .data$paralysis_ip >=1, 1, 0),
                    elix_neuro = dplyr::if_else(.data$neuro_ot >= 2 | .data$neuro_ip >=1, 1, 0),
                    elix_chronic_pulm = dplyr::if_else(.data$chronic_pulm_ot >= 2 | .data$chronic_pulm_ip >=1, 1, 0),
                    elix_diab_uc = dplyr::if_else(.data$diab_uc_ot >= 2 | .data$diab_uc_ip >=1, 1, 0),
                    elix_diab_c = dplyr::if_else(.data$diab_c_ot >= 2 | .data$diab_c_ip >=1, 1, 0),
                    elix_hypo = dplyr::if_else(.data$hypo_ot >= 2 | .data$hypo_ip >=1, 1, 0),
                    elix_renal = dplyr::if_else(.data$renal_ot >= 2 | .data$renal_ip >=1, 1, 0),
                    elix_liver = dplyr::if_else(.data$liver_ot >= 2 | .data$liver_ip >=1, 1, 0),
                    elix_peptic = dplyr::if_else(.data$peptic_ot >= 2 | .data$peptic_ip >=1, 1, 0),
                    elix_hiv = dplyr::if_else(.data$hiv_ot >= 2 | .data$hiv_ip >=1, 1, 0),
                    elix_lymph = dplyr::if_else(.data$lymph_ot >= 2 | .data$lymph_ip >=1, 1, 0),
                    elix_mets = dplyr::if_else(.data$mets_ot >= 2 | .data$mets_ip >=1, 1, 0),
                    elix_tumor = dplyr::if_else(.data$tumor_ot >= 2 | .data$tumor_ip >=1, 1, 0),
                    elix_rheum = dplyr::if_else(.data$rheum_ot >= 2 | .data$rheum_ip >=1, 1, 0),
                    elix_coag = dplyr::if_else(.data$coag_ot >= 2 | .data$coag_ip >=1, 1, 0),
                    elix_obese = dplyr::if_else(.data$obese_ot >= 2 | .data$obese_ip >=1, 1, 0),
                    elix_weightloss = dplyr::if_else(.data$weightloss_ot >= 2 | .data$weightloss_ip >=1, 1, 0),
                    elix_fluid = dplyr::if_else(.data$fluid_ot >= 2 | .data$fluid_ip >=1, 1, 0),
                    elix_bld_anem = dplyr::if_else(.data$bld_anem_ot >= 2 | .data$bld_anem_ip >=1, 1, 0),
                    elix_def_anem = dplyr::if_else(.data$def_anem_ot >= 2 | .data$def_anem_ip >=1, 1, 0),
                    elix_alcohol = dplyr::if_else(.data$alcohol_ot >= 2 | .data$alcohol_ip >=1, 1, 0),
                    elix_drug = dplyr::if_else(.data$drug_ot >= 2 | .data$drug_ip >=1, 1, 0),
                    elix_psych = dplyr::if_else(.data$psych_ot >= 2 | .data$psych_ip >=1, 1, 0),
                    elix_depression = dplyr::if_else(.data$depression_ot >= 2 | .data$depression_ip >=1, 1, 0))
  ...
  ## ICD-10 ----
    ...
    dat_comb <- dat_comb %>%
      dplyr::mutate(elix_aids = dplyr::if_else(.data$aids_ot >= 2 | .data$aids_ip >= 1, 1, 0),
                    elix_alcohol = dplyr::if_else(.data$alcohol_ot >= 2 | .data$alcohol_ip >= 1, 1, 0),
                    elix_def_anem = dplyr::if_else(.data$def_anem_ot >= 2 | .data$def_anem_ip >= 1, 1, 0),
                    elix_rheum = dplyr::if_else(.data$rheum_ot >= 2 | .data$rheum_ip >= 1, 1, 0),
                    elix_bld_loss = dplyr::if_else(.data$bld_loss_ot >= 2 | .data$bld_loss_ip >= 1, 1, 0),
                    elix_cancer_leuk = dplyr::if_else(.data$cancer_leuk_ot >= 2 | .data$cancer_leuk_ip >= 1, 1, 0),
                    elix_cancer_lymph = dplyr::if_else(.data$cancer_lymph_ot >= 2 | .data$cancer_lymph_ip >= 1, 1, 0),
                    elix_cancer_mets = dplyr::if_else(.data$cancer_mets_ot >= 2 | .data$cancer_mets_ip >= 1, 1, 0),
                    elix_cancer_solid = dplyr::if_else(.data$cancer_solid_ot >= 2 | .data$cancer_solid_ip >= 1, 1, 0),
                    elix_cancer_insitu = dplyr::if_else(.data$cancer_insitu_ot >= 2 | .data$cancer_insitu_ip >= 1, 1, 0),
                    elix_cbvd = dplyr::if_else(.data$cbvd_ot >= 2 | .data$cbvd_ip >= 1, 1, 0),
                    elix_chf = dplyr::if_else(.data$chf_ot >= 2 | .data$chf_ip >= 1, 1, 0),
                    elix_coag = dplyr::if_else(.data$coag_ot >= 2 | .data$coag_ip >= 1, 1, 0),
                    elix_dementia = dplyr::if_else(.data$dementia_ot >= 2 | .data$dementia_ip >= 1, 1, 0),
                    elix_depress = dplyr::if_else(.data$depress_ot >= 2 | .data$depress_ip >= 1, 1, 0),
                    elix_diab_c = dplyr::if_else(.data$diab_c_ot >= 2 | .data$diab_c_ip >= 1, 1, 0),
                    elix_diab_uc = dplyr::if_else(.data$diab_uc_ot >= 2 | .data$diab_uc_ip >= 1, 1, 0),
                    elix_drug = dplyr::if_else(.data$drug_ot >= 2 | .data$drug_ip >= 1, 1, 0),
                    elix_htn_c = dplyr::if_else(.data$htn_c_ot >= 2 | .data$htn_c_ip >= 1, 1, 0),
                    elix_htn_uc = dplyr::if_else(.data$htn_uc_ot >= 2 | .data$htn_uc_ip >= 1, 1, 0),
                    elix_liver_mild = dplyr::if_else(.data$liver_mild_ot >= 2 | .data$liver_mild_ip >= 1, 1, 0),
                    elix_liver_sev = dplyr::if_else(.data$liver_sev_ot >= 2 | .data$liver_sev_ip >= 1, 1, 0),
                    elix_chronic_pulm = dplyr::if_else(.data$chronic_pulm_ot >= 2 | .data$chronic_pulm_ip >= 1, 1, 0),
                    elix_neuro_movt = dplyr::if_else(.data$neuro_movt_ot >= 2 | .data$neuro_movt_ip >= 1, 1, 0),
                    elix_neuro_seiz = dplyr::if_else(.data$neuro_seiz_ot >= 2 | .data$neuro_seiz_ip >= 1, 1, 0),
                    elix_neuro_oth = dplyr::if_else(.data$neuro_oth_ot >= 2 | .data$neuro_oth_ip >= 1, 1, 0),
                    elix_obese = dplyr::if_else(.data$obese_ot >= 2 | .data$obese_ip >= 1, 1, 0),
                    elix_paralysis = dplyr::if_else(.data$paralysis_ot >= 2 | .data$paralysis_ip >= 1, 1, 0),
                    elix_perivasc = dplyr::if_else(.data$perivasc_ot >= 2 | .data$perivasc_ip >= 1, 1, 0),
                    elix_psych = dplyr::if_else(.data$psych_ot >= 2 | .data$psych_ip >= 1, 1, 0),
                    elix_pulmcirc = dplyr::if_else(.data$pulmcirc_ot >= 2 | .data$pulmcirc_ip >= 1, 1, 0),
                    elix_renal_mod = dplyr::if_else(.data$renal_mod_ot >= 2 | .data$renal_mod_ip >= 1, 1, 0),
                    elix_renal_sev = dplyr::if_else(.data$renal_sev_ot >= 2 | .data$renal_sev_ip >= 1, 1, 0),
                    elix_hypo = dplyr::if_else(.data$hypo_ot >= 2 | .data$hypo_ip >= 1, 1, 0),
                    elix_thyroid_oth = dplyr::if_else(.data$thyroid_oth_ot >= 2 | .data$thyroid_oth_ip >= 1, 1, 0),
                    elix_peptic = dplyr::if_else(.data$peptic_ot >= 2 | .data$peptic_ip >= 1, 1, 0),
                    elix_valve = dplyr::if_else(.data$valve_ot >= 2 | .data$valve_ip >= 1, 1, 0),
                    elix_weightloss = dplyr::if_else(.data$weightloss_ot >= 2 | .data$weightloss_ip >= 1, 1, 0))
  ...
  if (version == 9 | version == 19){
    ...
    dat4 <- dat4 %>%
      dplyr::rename(chf = elix_chf,
                    valve = elix_valve,
                    pulmcirc = elix_pulmcirc,
                    perivasc = elix_periph_vasc,
                    htn_c = elix_htn_comb,
                    para = elix_paralysis,
                    neuro = elix_neuro,
                    chrnlung = elix_chronic_pulm,
                    dm = elix_diab_uc,
                    dmcx = elix_diab_c,
                    hypothy = elix_hypo,
                    renlfail = elix_renal,
                    ...
                    coag = elix_coag,
                    obese = elix_obese,
                    wghtloss = elix_weightloss,
                    lytes = elix_fluid,
                    bldloss = elix_bld_anem,
                    anemdef = elix_def_anem,
                    alcohol = elix_alcohol,
                    drug = elix_drug,
                    psych = elix_psych,
                    depress = elix_depression)
    ...
    dat4 <- dat4 %>%
      dplr::rename(aids = elix_aids,
                   alcohol = elix_alcohol,
                   anemdef = elix_def_anem,
                   arth = elix_rheum,
                   bldloss = elix_bld_loss,
                   cancer_leuk = elix_cancer_leuk,
                   cancer_lymph = elix_cancer_lymph,
                   cancer_mets = elix_cancer_mets,
                   cancer_nsitu = elix_cancer_insitu,
                   cancer_solid = elix_cancer_solid,
                   cbvd = elix_cbvd,
                   chf = elix_chf,
                   coag = elix_coag,
                   dementia = elix_dementia,
                   depress = elix_depress,
                   diab_cx = elix_diab_c,
                   diab_uncx = elix_diab_uc,
                   drug_abuse = elix_drug,
                   htn_cx = elix_htn_c,
                   htn_uncx = elix_htn_uc,
                   liver_mld = elix_liver_mild,
                   liver_sev = elix_liver_sev,
                   lung_chronic = elix_chronic_pulm,
                   neuro_movt = elix_neuro_movt,
                   neuro_oth = elix_neuro_seiz,
                   neuro_seiz = elix_neuro_oth,
                   obese = elix_obese,
                   paralysis = elix_paralysis,
                   perivasc = elix_perivasc,
                   psychoses = elix_psych,
                   pulmcirc = elix_pulmcirc,
                   renlfl_mod = elix_renal_mod,
                   renlfl_sev = elix_renal_sev,
                   thyroid_hypo = elix_hypo,
                   thyroid_oth = elix_thyroid_oth,
                   ulcer_peptic = elix_peptic,
                   valve = elix_valve,
                   wghtloss = elix_weightloss)
